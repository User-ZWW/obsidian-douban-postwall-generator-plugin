...
var getHtmlTemplate = (settings, imageCount, jsonContent) => `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
`;
var PosterWallPlugin = class extends import_obsidian.Plugin {
  async onload() {
    ...
    this.registerEvent(
      this.app.workspace.on("file-menu", (menu, file) => {
        if (file instanceof import_obsidian.TFolder) {
          menu.addItem((item) => {
            item.setTitle("Generate Poster Wall").setIcon("zap").onClick(async () => await this.generatePosterWallForFolder(file));
          });
        }
      })
    );
  ...
  async generatePosterWallForFolder(folder) {
    new import_obsidian.Notice(`Generating ${folder.name} Poster Wall...`);
    try {
      const images = [];
      const validExtensions = ["jpg", "jpeg", "png", "webp", "gif"];
      const getName = (name) => name.substring(0, name.lastIndexOf("."));
      for (const child of folder.children) {
        if (child instanceof import_obsidian.TFile && validExtensions.includes(child.extension.toLowerCase())) {
          images.push({
            id: child.name,
      ...
      if (images.length === 0) {
        new import_obsidian.Notice("No images found.");
        return;
      }
      ...
      new import_obsidian.Notice("Poster Wall Generated!");
    } catch (error) {
      console.error(error);
      new import_obsidian.Notice("Error generating wall.");
    }
  }
};
var PosterWallSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Wall Title").addText((text) => text.setValue(this.plugin.settings.wallTitle).onChange(async (v) => {
      this.plugin.settings.wallTitle = v;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Description").addText((text) => text.setValue(this.plugin.settings.wallDescription).onChange(async (v) => {
      this.plugin.settings.wallDescription = v;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Avatar URL").addText((text) => text.setValue(this.plugin.settings.avatarUrl).onChange(async (v) => {
      this.plugin.settings.avatarUrl = v;
      await this.plugin.saveSettings();
    }));
    containerEl.createEl("h3", { text: "Default Layout" });
    new import_obsidian.Setting(containerEl).setName("Card Width").setDesc("Width of each poster in pixels").addSlider((slider) => slider.setLimits(100, 400, 10).setValue(this.plugin.settings.defaultCardWidth).setDynamicTooltip().onChange(async (v) => {
      this.plugin.settings.defaultCardWidth = v;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Spacing (Gap)").setDesc("Gap between posters").addSlider((slider) => slider.setLimits(0, 100, 2).setValue(this.plugin.settings.defaultGap).setDynamicTooltip().onChange(async (v) => {
      this.plugin.settings.defaultGap = v;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Corner Radius").setDesc("Roundness of corners").addSlider((slider) => slider.setLimits(0, 50, 1).setValue(this.plugin.settings.defaultRadius).setDynamicTooltip().onChange(async (v) => {
      this.plugin.settings.defaultRadius = v;
      await this.plugin.saveSettings();
    }));
    containerEl.createEl("h3", { text: "Features" });
    new import_obsidian.Setting(containerEl).setName("Show Titles by Default").addToggle((toggle) => toggle.setValue(this.plugin.settings.showTitlesByDefault).onChange(async (v) => {
      this.plugin.settings.showTitlesByDefault = v;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Enable Edit Mode by Default").addToggle((toggle) => toggle.setValue(this.plugin.settings.enableEditModeByDefault).onChange(async (v) => {
      this.plugin.settings.enableEditModeByDefault = v;
      await this.plugin.saveSettings();
    }));
  }
};
